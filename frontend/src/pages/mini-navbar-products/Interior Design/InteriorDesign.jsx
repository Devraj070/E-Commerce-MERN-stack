  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const filteredProducts = response.data.filter(item => item.category === "interior-design");
        setinteriorDesign(filteredProducts);
      } catch (error) {
      ...
      {loading ? (
        ...
        <>
          <div className='flex bg-cyan-600'>
            <div className='text-lg p-2 mt-52 ml-20 mb-80 rounded-lg bg-black text-white hover:bg-green-500 cursor-pointer'>Book Now</div>
          </div>


          <h1 className="text-2xl font-bold bg-white pl-4">our previous works</h1>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-0">
            {InteriorDesign.map((product) => (
