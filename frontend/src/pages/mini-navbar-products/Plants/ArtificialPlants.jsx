...
const ArtificialPlants = () => {
    ...
    const [artificialPlants, setArtificialPlants] = useState([]);
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // const plant = response.data.filter((item) => item.category === "ArtificialPlants")
                // console.log("ArtificialPlants:", plant);
                // setArtificialPlants(plant);
                const response = await axios.get('http://localhost:3006/api/products');
                const products = response.data;
                const filteredProducts = products.filter(item => item.category === "artificial-plants");
                setArtificialPlants(filteredProducts);
            } catch (error) {
