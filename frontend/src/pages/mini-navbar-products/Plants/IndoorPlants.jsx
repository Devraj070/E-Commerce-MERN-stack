    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // const plant = response.data.filter((item) => item.category === "IndoorPlants")
                // console.log("IndoorPlants:", plant);
                // setIndoorPlants(plant);
                const response = await axios.get('http://localhost:3006/api/products');
                const products = response.data;
                const filteredProducts = products.filter(item => item.category === "indoor-plants");
                setIndoorPlants(filteredProducts);
            } catch (error) {
