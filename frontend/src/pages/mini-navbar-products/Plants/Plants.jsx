  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // const plant = response.data.filter((item) => item.category === "plants")
        // console.log("Plants:", plant);
        // setPlants(plant);
        const response = await axios.get('http://localhost:3006/api/products');
        const products = response.data;
        const filteredProducts = products.filter(item => item.category === "plants" || item.category === "artificial-plants" || item.category === "succulents" || item.category === "indoor-plants");
        setPlants(filteredProducts);
      } catch (error) {
