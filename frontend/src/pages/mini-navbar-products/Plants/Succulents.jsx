    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // const plant = response.data.filter((item) => item.category === "Succulents")
                // console.log("Succulents:", plant);
                // setSucculents(plant);
                const response = await axios.get('http://localhost:3006/api/products');
                const products = response.data;
                const filteredProducts = products.filter(item => item.category === "succulents");
                setSucculents(filteredProducts);
            } catch (error) {
