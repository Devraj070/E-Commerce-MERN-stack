...
// Products component
const PatternedWallpaper = () => {
  const [patternedWallpaper, setPatternedWallpaper] = useState([]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const filteredProducts = response.data.filter(item => item.category === "patterned-wallpaper")
        setPatternedWallpaper(filteredProducts);
      } catch (error) {
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-0">
            {patternedWallpaper.map((product) => (
...
export default PatternedWallpaper;
